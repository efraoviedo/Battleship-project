...
var output = {
	"welcome": " Play BattleShip. If play along... with the computer... Select the menu above to get started.",
	"not": "  Settings not  available.",
	"player1": " Place your own ships or the computer randomly do it?",
	"self": "  Click the Horizontal or Vertical on the buttons to set your ships on the bottom square.",
	"overlap": " No more ships available.  Please try again.",
	"start": " Click on the square to fire on the enemy's square.  Have Fun!",
	...
	lost: function(name) { return " > " + name + " has lost his fleet!!  Game Over Refresh to start again." },
};
...
$(document).ready(function() {
	for (var i = 1; i <= 100; i++) {
		...
		if (i < 11) {
			...
			$(".grid").append("<li class='points offset1 " + i + "'><span class='X'></span></li>");
		} else {
			$(".grid").append("<li class='points offset2 " + i + "'><span class='x'></span></li>");
		}
		if (i == 11) {
...
function gameSetup(t) {
	$(t).off() && $(".two").off();
	$(".one").addClass("self").removeClass("one").text("Place My ships");
...
function selfSetup() {
	$(".self").addClass("horz").removeClass("self").text("Horizontally");
